                                        ...
                                        for Po in Pos:
                                            for R in Rs:
                                                for sd in sds:
                                                    ...
                                                    s = "#!/bin/bash\n#PBS -N %s\n#PBS -l nodes=2:ppn=8,walltime=1:05:00\n\ncd $PBS_O_WORKDIR\nexport OMP_NUM_THREADS=16\n" % filename
                                                    s = s + "./main.exe " + str(N) + " " + str(Nl) + " " + str(charge) + " " + str(ep) + " " + str(h) + " "  + str(ht) + " " + str(r0) + " " + str(T) + " " + str(tF) + " " + str(R) + " " + filename + " " + str(Po) + " 1000 " + str(sd)
                                                    f.write(s)
                                                    f.close
                                                    i=i+1
...
generate_jobs([32],[16,32],[4.0],[0.5,0.05],[1.0],[1.0],[0.4],[3000000],[0.001], [0.2, 0.6, 1.0, 1.4, 1.8, 2.2, 2.6, 3.0, 3.4, 3.8, 4.2, 4.6, 5.0, 5.4, 5.8], [1000], [1,2,3])
